...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        # Defined pass/fail criteria per scenario:
        if scenario.name == "primary_timeout_100":
            passed = result.fallback_successes > 0 or result.availability >= 0.7
        elif scenario.name == "primary_flaky_50":
            passed = result.circuit_open_count > 0 or result.availability >= 0.7
        elif scenario.name == "all_healthy":
            passed = result.availability >= 0.9
        elif scenario.name == "cascading_failure_backup_flaky":
            passed = result.static_fallbacks > 0 or result.successful_requests > 0
        else:
